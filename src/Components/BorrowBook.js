 ...
 getBookById() {
 axios.get('http://localhost:4000/books/editBook/' + this.props.match.params.id)
 .then((response) => {
 this.setState({
 ...
 const { title, author, donatedbyName, note, borrowedStatus, borrowedbyName } = this.state;
 axios.post('http://localhost:4000/books/updateBook/' + this.props.match.params.id, {
 title: title,
 author: author,
 ...
 <br />
 <label>
 Donated By : {this.state.donatedbyName}
 </label>
 <br />
 <label>
 About: {this.state.note}
 </label>
 <br />
  <label>
 Borrowed By
