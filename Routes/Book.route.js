...
bookRoute.route("/updateBook/:id").post(function (req, res) {
  bookModel.findById(req.params.id, function (err, book) {
    if (!book) return next(new Error("Unable To Find Book With This Id"));
    else {
...
bookRoute.route("/searchBook/:searchTitle").get(function (req, res) {
  bookModel.find(
    { title: req.params.searchTitle, borrowedStatus: 0 },
    function (err, book) {
      if (err) {
        console.log(err);
      } else {
...
bookRoute.route("/searchAuthor/:searchAuthor").get(function (req, res) {
  bookModel.find(
    { author: req.params.searchAuthor, borrowedStatus: 0 },
    function (err, book) {
      if (err) {
        console.log(err);
      } else {
...
bookRoute.route("/searchUser/:searchUser").get(function (req, res) {
  bookModel.find(
    { borrowedbyName: req.params.searchUser, borrowedStatus: 1 },
    function (err, book) {
      if (err) {
        console.log(err);
      } else {
